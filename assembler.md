...
1. Логічні команди процессора

До логічних команд відносять наступні:
- AND (Виконує операцію логічного І між двома операндами)
- OR (Виконує операцію логічного АБО між двома операндами)
- NOT (Виконує операцію логічного заперечення (НІ) одного операнда
- XOR (Виконує операцію виключного АБО між двома операндами)

2. Команди обробки бітів

Крім логічних команд, до команд обробки бітів належать:
- BT - Переносить біт у CF
- BTR - Переносить біт у CF та задає йому 0
- BTC - Переносить біт в CF та інвертує його
- BSF - Передивляється біти другого операнда від молодшого до старшого у пошуку одиниці
- BSR - Аналогічно до BSF, але передивляється в зворотньому порядку

3. Команди переміщення даних

До команд переміщення даних відносять команди:
- MOV куди, звідки
- XCHG операнд1, операнд2

Основною командою переміщення даних є команда MOV. Вона реалізує різноманітні варіанти переміщення, в залежності від переданих операндів.

Команда XCHG використовується для обміну значень місцями (двунаправлене переміщення).

## Контрольная работа №2

1. Команди роботи з двійковими числами

- INC - Збільшення значення операнда на 1
- DEC - Зменшення значення операнда на 1
- ADD - Додавання другого операнда до першого
- ADC - Додавання другого операнда та значення CF до першого 
- SUB - Віднімання від першого операнда другого
- SBB - Віднімання від першого операнда другого та значення CF
- MUL - Беззнакове множення значення акумулятора та операнда
- IMUL - Знакове множення значення акумулятора та операнда
- DIV - Беззнакове ділення акумулятора на операнд
- IDIV - Знакове ділення акумулятора на операнд
- XADD - Обмін значень місцями та складання
- NEG - Заперечення з доповненням до двох

2. Команди передачі управління

- JMP - Безумовний перехід
- CMP - Порівняння операндів для наступного умовного переходу
- JE - Перехід якщо операнди порівняння рівня
- JNE - Якщо операнди різні
- JL - Якщо перший операнд менший
- JLE - Якщо перший операнд менший або дорівнює другому
- JG - Якщо перший операнд більший
- JGE - Якщо перший операнд більший або дорівнює другому
- NEAR - Прямий перехід в межах сегменту
- FAR - Прямий перехід у інший сегмент
- WORD - Косвений перехід в межах сегменту
- DWORD - Косвений перехід в інший сегмент
- RET - Передача управління викликаючій програмі
- LOOP - Створення циклу
...
- Регістрова
- Косвена регістрова
- Косвена регістрова зі зміщенням
- Косвена базова індексна
- Косвена базова індексна зі зміщенням
